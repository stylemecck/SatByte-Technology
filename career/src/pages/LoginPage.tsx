...
import { Mail, Lock, LogIn, ArrowRight, Loader2, ChevronLeft, X, Rocket } from 'lucide-react'
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-6 relative overflow-hidden selection:bg-brand-blue/30">
      ...
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-blue/20 blur-[120px] rounded-full translate-x-1/2 -translate-y-1/2 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-brand-emerald/10 blur-[120px] rounded-full -translate-x-1/2 translate-y-1/2 pointer-events-none" />
      ...
      >
        <div className="text-center mb-12">
          <Link to="/" className="inline-flex flex-col items-center group">
             ...
             <h2 className="text-4xl font-black text-foreground tracking-tighter mb-2 leading-none uppercase">Member Access</h2>
             <p className="text-muted-foreground font-bold text-xs uppercase tracking-[0.3em]">Synchronize your profile.</p>
          </Link>
        </div>
        ...
        <div className="glass p-12 rounded-[3.5rem] border border-border shadow-2xl relative">
          ...
          <form className="space-y-10" onSubmit={handleSubmit}>
            <div className="space-y-4">
              <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground ml-2">Secure Identity (Email)</label>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between ml-2">
                <label className="block text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">Private Key</label>
                <Link to="/" className="text-[10px] font-black text-brand-blue uppercase tracking-widest hover:underline decoration-2 underline-offset-4">Reset</Link>
              </div>
              <div className="relative group">
            ...
            >
              {isLoading ? <Loader2 className="animate-spin" size={24} /> : <><LogIn size={24} className="group-hover/btn:-translate-x-1 transition-transform" /> Initiate Session</>}
            </button>
          </form>
          ...
          <div className="mt-12 pt-10 border-t border-border text-center">
            <p className="text-sm font-bold text-muted-foreground">
              ...
              <Link to="/register" className="font-black text-brand-blue hover:underline decoration-2 underline-offset-4 inline-flex items-center gap-2 uppercase tracking-widest text-[11px] ml-2">
